...
featureNamesData <- read_table("UCI HAR Dataset/features.txt", col_names = c("Index","Feature"))
...
testfileList <- list.files("./UCI HAR Dataset/test/", full.names = TRUE,pattern = "*.txt")
testfileList # subject_test.txt is 1, X_test.txt is 2 and Y_test.txt is 3
# Read in the test data
subject_test_data <- read_table(testfileList[1],col_names = ("Subject ID"))
...
testData <- bind_cols(subject_test_data,y_TestSet,x_TestSet_selected)
# View(testData)

rm(list=c("subject_test_data","x_TestSet", "y_TestSet","testfileList","x_TestSet_selected"))
# Import Training Data, select only standard deviations and means ---------------
traningFileList <- list.files("./UCI HAR Dataset/train/",full.names = TRUE,pattern = "*.txt")
traningFileList # Data files are in the same order
subject_train_data <- read_table(traningFileList[1],col_names = c("Subject ID"))
x_TrainSet <- read_table(traningFileList[2],col_names = FALSE)
colnames(x_TrainSet) <- featureNames
x_TrainSet_selected <- x_TrainSet %>% 
  select(contains("mean")|contains("std"))
y_TrainSet <- read_table(traningFileList[3],col_names = c("Activity"))
trainData <- bind_cols(subject_train_data,y_TrainSet,x_TrainSet_selected)
rm(list=c("subject_train_data","x_TrainSet_selected","y_TrainSet","traningFileList","x_TrainSet", "featureNames"))
# View(trainData)
# Combine data and label activities-----------
activityLabels <- read_table("UCI HAR Dataset/activity_labels.txt", col_names = c("Index","Activity"))
View(activityLabels)
...
# View(combinedData)
meanCombinedData <- combinedData %>% summarise_all(mean)
meanCombinedData %>% write_csv("230125_meanCombinedData.csv")
rm(list=c("testData","trainData","activityLabels"))
